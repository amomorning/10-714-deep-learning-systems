...
#define TILE 8
                 ...
                 std::vector<uint32_t> strides, size_t offset)
    {
      ...
      {
        int cur = n - 1;
        while (cur >= 0 && idx[cur] == shape[cur] - 1)
        {
          idx[cur] = 0;
          cur -= 1;
        }
                     ...
                     uint32_t n, uint32_t p)
    {
      ...
      for (int i = 0; i < m / TILE; ++i)
      {
        for (int j = 0; j < n / TILE; ++j)
        {
          ...
          {
            Compact(b, &buffer, {TILE, TILE}, {1, TILE}, j * p * TILE + k * TILE * TILE);
            AlignedDot(
                a.ptr + i * n * TILE + j * TILE * TILE,
                buffer.ptr,
